...
if (isset($_GET['p_id'])) {
    ...
    if ($stmtPD_D->rowCount() != 1) {
        header('location: ../dealer_general/dealer_gen_product.php');
        exit();
    }
}
        ...
        if (!empty($rowCheck)) {
            foreach ($rowCheck as $rowCheck) {
                if ($rowCheck['product_id'] == $p_id) {
                    ...
                    header("refresh:2; url=dealer_gen_product_detail.php?p_id=$p_id");
                    if ($check_state == true) {
                        break;
                    }
                } else {
            ...
            if ($check_state == false) {
                ...
                if ($sql) {
                    ...
                    header("refresh:0.0000000001; url=dealer_gen_product_detail.php?p_id=$p_id");
                }
            }
        } else {
            $sql = $conn->prepare("INSERT INTO cart(qty,price,product_id, user_id, status_cart)
        ...
        if (!empty($rowCheck)) {
            foreach ($rowCheck as $rowCheck) {
                if ($rowCheck['product_id'] == $p_id) {
                    ...
                    // })
                    // </script>";
                    header("refresh:0.0000001; url=dealer_gen_cart.php");
                    if ($check_state == true) {
                        break;
                    }
                } else {
            ...
            if ($check_state == false) {
                ...
                if ($sql) {
                    ...
                    // </script>";
                    $check_state = true;
                    header("refresh:1; url=dealer_gen_cart.php");
                } else {
                    echo "<script>alert('มีบางอย่างผิดพลาด')</script>";
                    header("refresh:0.0000000001; url=dealer_gen_cart.php");
                }
            }
        } else {
            $sql = $conn->prepare("INSERT INTO cart(qty,price,product_id, user_id, status_cart)
            ...
            <div class="g-box-related">
                ...
                foreach ($rowRT as $rowRT) { ?>
                    <div class="related-RT">
                        <a class="a-none" href="../dealer_general/dealer_gen_product_detail.php?p_id=<?= $rowRT['p_id']; ?>">
                            <div class="card" id="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <a href="../dealer_general/dealer_gen_product_detail.php?p_id=<?= $rowRT['p_id']; ?>" class="btn " id="btn-dp" style="margin: 0 auto;">รายละเอียด</a>
                                </div>
                            </div>
                        </a>
                    </div>
